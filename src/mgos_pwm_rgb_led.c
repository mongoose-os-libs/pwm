...
typedef struct params_s {
  ...
  bool common_cathode;  // COMMON CATHODE (ie they have a common ground ). They
                        // turn off with PWM at 0
...
static void vLEDPWMTask(void *pvParameters) {
  ...
  */
  ...
  if (!pParams->common_cathode) {
    // COMMON CATHODE (ie they have a common ground ). They turn off with PWM at
    // 0 COMMON ANODE means the LEDs share a common voltage source / VIN. They
    // turn off with PWM at 1
    offDuty = 1;
  }
  ...
  // Note xFrequency is in ticks, not ms. So we convert from ms by dividing by
  // portTICK_PERIOD_MS
  ...
  if (pParams->led1_gpio) {
    ...
    // This is which is super annoying if you have a common anode LED that needs
    // 1 to turn off
    mgos_gpio_write(pParams->led1_gpio, offDuty ? 1 : 0);
  }
  if (pParams->led2_gpio) {
    mgos_pwm_set(pParams->led2_gpio, 0, offDuty);
    mgos_gpio_write(pParams->led2_gpio, offDuty ? 1 : 0);
  }
  ...
  // pParams->led2_pct, pParams->led3_pct, pParams->freq);
  LOG(LL_DEBUG, ("LEDPWMTASK int r(%d) %3.2f, g(%d) %3.2f, b(%d) %3.2f freq %d",
                 pParams->led1_gpio, pParams->led1_pct, pParams->led2_gpio,
                 pParams->led2_pct, pParams->led3_gpio, pParams->led3_pct,
                 (int) pParams->freq));
  ...
  for (;;) {
    ...
    */
    ...
    if (pParams->led1_gpio)
      mgos_pwm_set(pParams->led1_gpio, pParams->freq,
                   on ? pParams->led1_pct : offDuty);
    if (pParams->led2_gpio)
      mgos_pwm_set(pParams->led2_gpio, pParams->freq,
                   on ? pParams->led2_pct : offDuty);
    if (pParams->led3_gpio)
      mgos_pwm_set(pParams->led3_gpio, pParams->freq,
                   on ? pParams->led3_pct : offDuty);
...
static void mgos_pwm_rgb_fade_set(int chan, float pct, int ms,
                                  bool common_cathode) {
  ...
  ledc_set_fade_with_time(LEDC_HIGH_SPEED_MODE, (ledc_channel_t) chan,
                          (int) duty, ms);

  ledc_fade_start(LEDC_HIGH_SPEED_MODE, (ledc_channel_t) chan,
                  LEDC_FADE_NO_WAIT);
...
static bool mgos_pwm_rgb_led_apply(struct mgos_pwm_rgb_led *led) {
  ...
  LOG(LL_DEBUG,
      ("Input LED R %d, G %d, B %d, BR %d", led->r, led->g, led->b, led->br));
  ...
  if (led->common_cathode) {
    ...
    LOG(LL_DEBUG,
        ("CATHODE apply r %g, g %g, b %g | brightness %g", rv, gv, bv, brv));
  } else {
    // COMMON ANODE means the LEDs share a common voltage source / VIN
    // User supplies (0 = off, 1 = fully on)
    // If common anode, we need to flip the values as a 0 output = fully on / 1
    // = off
    rv = 1.0f - rv;
    gv = 1.0f - gv;
    bv = 1.0f - bv;
  ...
  // Save it so we can call apply then use the resulting values elsewhere, ie
  // pwm blinking
  led->gpio_r_pct = rv;
  led->gpio_g_pct = gv;
  led->gpio_b_pct = bv;

  LOG(LL_DEBUG, ("Apply saving pct r %g, g %g, b %g ", led->gpio_r_pct,
                 led->gpio_g_pct, led->gpio_b_pct));

  if (led->fade_direction > FADE_OFF) {
    LOG(LL_DEBUG, ("Calling fade set"));
    if (led->r && led->gpio_r_chan > -1)
      mgos_pwm_rgb_fade_set(led->gpio_r_chan, rv, led->time_on,
                            led->common_cathode);
    if (led->g && led->gpio_g_chan > -1)
      mgos_pwm_rgb_fade_set(led->gpio_g_chan, gv, led->time_on,
                            led->common_cathode);
    if (led->b && led->gpio_b_chan > -1)
      mgos_pwm_rgb_fade_set(led->gpio_b_chan, bv, led->time_on,
                            led->common_cathode);
...
void mgos_pwm_rgb_channel_update_cb(int ev, void *ev_data, void *userdata) {
  // Note, after calling the first mgos_pwm_set you should wait ~300ms for the
  // channels to update and fire the event
  ...
  if ((int) data->channel >= 0) {
    LOG(LL_DEBUG, ("pwm_channel_update_cb: pin %d is on chan %d", data->pin,
                   data->channel));
...
void mgos_pwm_rgb_blink_task(void *ledArg) {
  ...
  LOG(LL_DEBUG,
      ("LEDC Blinking LED via pwm rgb blink task, time %d ", led->time_on));

  // Very simple, we just need to call our function then delay however long we
  // require If you just use the 'best effort' timers the blinking is
  // interrupted by other timers running in a single thread
  while (1) {
    ledc_fade_cb(&led);
...
void mgos_pwm_rgb_blink_start(struct mgos_pwm_rgb_led *led, int ms_on,
                              int ms_off) {
  ...
  LOG(LL_INFO,
      ("LEDC blink_start time on: %d off: %d", led->time_on, led->time_off));
  ...
  xReturned = xTaskCreatePinnedToCore(
      ...
      1024 * 4,     /* Stack size in words, not bytes.  ..which might not be the
                       case anymore according to espressif */
...
void mgos_pwm_rgb_fade_start(struct mgos_pwm_rgb_led *led, int ms,
                             enum mgos_pwm_fade_direction direction,
                             bool resetToStartingPosition, uint8_t max,
                             uint8_t min) {
  ...
  LOG(LL_DEBUG, ("LEDC fade installed. time %d, direction %d", led->time_on,
                 led->fade_direction));

  if (direction == FADE_UP) {
    if (resetToStartingPosition) {
      LOG(LL_DEBUG,
          ("LEDC UP trying to set brightness with led & min %u ", min));
    ...
    if (resetToStartingPosition) {
      LOG(LL_DEBUG,
          ("LEDC DOWN trying to set brightness with led & max %u ", max));
    ...
    mgos_clear_timer(led->led_timer_id);  // otherwise with multiple fades we
                                          // end up with multiple timers!
    led->led_timer_id =
        mgos_set_timer(led->time_on + 1 /* so we know it finished */,
                       MGOS_TIMER_REPEAT, ledc_fade_cb, led);
  }
}
